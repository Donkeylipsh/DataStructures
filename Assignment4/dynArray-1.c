 ...
 * Date: 3/2/2018
 * Description: dynArray.c is a Dynamic Array data structure that has been modified to include Heap implementation
 ************************************************************************************************/
...
{
	// Validate heap parameter
	assert(heap != 0);
	assert(heap->data != 0);

  	// Return the value stored at heap->data[0] to temp
...
void addHeap(DynArr *heap, TYPE node)
{
	...
	// Percolate value up to the appropriate index
	int parentIndex = ( (heap->size) / 2 ) - 1;
	int currentIndex = heap->size - 1;

    // Check if value stored in currentIndex < is less than the value in the parent index
	while ( (parentIndex >= 0) && ( currentIndex = _smallerIndexHeap(heap, parentIndex, currentIndex) ) ){
        // Swap values
		swapDynArr(heap, parentIndex, currentIndex);

		// Update current and parent index for next loop
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	if (rightChild < max) {
		// Get the index of the smallest child value
		if (leftChild == _smallerIndexHeap(heap, leftChild, rightChild)) {
		...
		// Check if the value stored at smallestChild is less than the value at pos
		if ( smallestChild == _smallerIndexHeap(heap, smallestChild, pos) ) {
			// Swap the values
			swapDynArr(heap, pos, smallestChild);

			// Call adjustHeap on smallestChild
			_adjustHeap(heap, max, smallestChild);
		}
	}
	else if (leftChild < max) {
		// If not then, left child is the smallestChild
		smallestChild = leftChild;

		// Check if the value stored at smallest child is less than the value at pos
		if ( smallestChild == _smallerIndexHeap(heap, smallestChild, pos) ) {
			// Swap the values
			swapDynArr(heap, pos, smallestChild);
		...
		// There are no children so nothing to adjust
	}
}
...
void removeMinHeap(DynArr *heap)
{
  	...
  	// Swap value at top of heap with last value in heap
  	swapDynArr(heap, 0, ( heap->size - 1) );

  	// Remove th last value in the heap
  	removeAtDynArr(heap, (heap->size - 1) );

  	// Call _adjustHeap to place the new top value at the appropriate position
...
void _buildHeap(DynArr *heap)
{
	...
	// Assign max index for heap
	int max = heap->size;

	// Loop through each parent node index of arbitrary dynArray
	for( int i = ( max / 2 ) - 1; i >= 0; i--){
        // Use _adjustHeap to position value stored at index i (and it's heap children) of arbitrary dynArry at its appropriate heap index
...
void sortHeap(DynArr *heap)
{
	...
	// Build a heap from an arbitrary dynArray
	_buildHeap(heap);

	// Sort the heap in reverse sorted order
	for(int i = (heap->size - 1); i >= 0; i--){
        // Swap first element and last unsorted element
        swapDynArr(heap, 0, i);

        // Use adjust heap to move smallest unsorted value to index i
